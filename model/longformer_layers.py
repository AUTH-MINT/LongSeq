...
from model.longformer_diagonaled_mm_tvm import diagonaled_mm as diagonaled_mm_tvm, mask_invalid_locations
from model.longformer_sliding_chunks import sliding_chunks_matmul_qk, sliding_chunks_matmul_pv
from model.longformer_sliding_chunks import sliding_chunks_no_overlap_matmul_qk, sliding_chunks_no_overlap_matmul_pv
import math
import torch
from torch import nn
import torch.nn.functional as F

from model.linformer_reversible import ReversibleSequence, SequentialSequence
...
class LongformerSelfAttention(nn.Module):
    def __init__(self, dim, heads, layer_id, dropout, attention_window, attention_dilation, attention_mode, autoregressive):
        super().__init__()
        if dim % heads != 0:
            raise ValueError(
        ...
        self.query_global = nn.Linear(dim, self.embed_dim)
        self.key_global = nn.Linear(dim, self.embed_dim)
        self.value_global = nn.Linear(dim, self.embed_dim)
...
class Longformer(nn.Module):
    def __init__(self, dim, depth, heads, dropout, attention_window, attention_dilation,
                 autoregressive, attention_mode):
        """
                     Args:
        ...
        for i in range(depth):
            attn = LongformerSelfAttention(dim, heads, i, dropout, attention_window, attention_dilation, attention_mode, autoregressive)
            ff = FeedForward(dim)
